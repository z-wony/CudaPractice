...
void _createXMap(int width, int height, Npp32f **xMap, int *steps)
{
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            int count = (int)(c * ((float)width / height));
            if (x < (width - count) / 2)
                cpuMem[y * width + x] = 0.0f;
            else if (x >= (width + count) / 2)
                cpuMem[y * width + x] = (float)(width - 1);
            else {
                if (x < width / 2) {
                    cpuMem[y * width + x] = (float)(((x - (width - count) / 2) / (float)count) * width);
                    
                } else {
                    cpuMem[y * width + x] = (float)(((x - width / 2) * 2 / (float)count) * width / 2 + width / 2);
                }
            }
        }
    }
